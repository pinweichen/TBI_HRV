...
transcr<-sbdt[grep("tra", sbdt$files),]
transcr<-transcr[!(files %in% "transcribe"),]
setwd(paste0(data_path))
fwrite(transcr,"folder_incorrect_list.csv")

rm(lsfa,lsfadt,ls_subfd,dt, sbdt,transcr)
...
for (it in 1:length(ls_fd)){
  p <- ls_fd[it]
  anne_p <- paste0(ANNE_p,"/",p,"/transcribe/")
  setwd(anne_p)
  #browser()
